...
//		btnSalvar.setOpaque(true);
		...
		txtCpf.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent event) {
				if(event.getKeyCode() == KeyEvent.VK_ENTER) {
					Cliente cliente = clienteController.consultarPorCpf(txtCpf.getText());
					if(cliente != null) {
						//preencher os dados
						txtNome.setText(cliente.getNome());
						int idEndereco = cliente.getEndereco().getId();
						int posicaEndereco = -1;
						for(int i = 0; i < cbEndereco.getItemCount(); i++) {
							Endereco endereco = (Endereco) cbEndereco.getItemAt(i);
							if(endereco.getId() == idEndereco)
								posicaEndereco = i;
						}
						System.out.println(posicaEndereco);
						cbEndereco.setSelectedIndex(posicaEndereco);
					}
				}
			}
		});
